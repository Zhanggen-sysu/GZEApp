...
#import "GZEAudioEncoder.h"
#import "Macro.h"
#import <AudioToolbox/AudioToolbox.h>

@interface GZEAudioEncoder ()
{
    // 待编码缓冲区
    char *_leftBuffer;
    // 待编码缓冲区的长度，动态
    NSInteger _leftLength;
    // 编码缓冲区
    char *_aacBuffer;
    // 每次送给编码器的数据长度
...
// 音频编码器实例
@property (nonatomic, assign) AudioConverterRef audioEncoderInstance;
// 音频编码参数
@property (nonatomic, assign) CMFormatDescriptionRef aacFormat;
// 音频编码码率
@property (nonatomic, assign, readwrite) NSInteger audioBitrate;
@property (nonatomic, assign) BOOL isError;
@property (nonatomic, strong) dispatch_queue_t encoderQueue;

@end

@implementation GZEAudioEncoder

- (instancetype)initWithAudioBitrate:(NSInteger)audioBitrate
{
    if (self = [super init]) {
        _audioBitrate = audioBitrate;
        _encoderQueue = dispatch_queue_create("com.GZEApp.audioEncoder", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
                                                ...
                                                error:(NSError **)error
{
    ...
    // AAC 编码格式。常用的 AAC 编码格式：kAudioFormatMPEG4AAC、kAudioFormatMPEG4AAC_HE_V2
    outputFormat.mFormatID = kAudioFormatMPEG4AAC;
    // 输出声道数与输入一致
    outputFormat.mChannelsPerFrame = (UInt32)inputFormat.mChannelsPerFrame;
    // 每个包的帧数。AAC 固定是 1024，这个是由 AAC 编码规范规定的。对于未压缩数据设置为 1
...
- (void)encodeSampleBuffer:(CMSampleBufferRef)buffer
{
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        [self encodeSampleBufferInternal:buffer];
        CFRelease(buffer);
    });
}
...
- (void)encodeSampleBufferInternal:(CMSampleBufferRef)buffer
{
    ...
    // 获取音频参数信息，AudioStreamBasicDescription 包括了音频的数据格式、声道数、采样位深、采样率等
    ...
    if (_leftLength + audioLength >= _bufferLength) {
        // 当待编码缓冲区遗留数据加上新来的数据长度(_leftLength + audioLength)大于每次给编码器的数据长度(_bufferLength)时，
        // 则进行循环编码，每次送给编码器长度为 _bufferLength 的数据量。
        // 拷贝待编码的数据到缓冲区 totalBuffer。
        // 当前总数据长度
        NSInteger totalSize = _leftLength + audioLength;
        // 计算给编码器送数据的次数
        NSInteger encodeCount = totalSize / _bufferLength;
...
- (void)encodeBuffer:(char *)buffer timing:(CMSampleTimingInfo)timing
{
    ...
    // 需要在回调方法 inputDataProcess 中将待编码的数据到编码器的缓冲区的对应位置。这里把我们自己创建的待编码缓冲区 AudioBufferList 作为 inInputDataProcUserData 传入，在回调方法中直接拷贝它。
